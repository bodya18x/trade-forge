...
Реализованы следующие возможности:
    - Получение данных по API с асинхронными запросами.
    - Обработка различных ошибок и исключений.
    - Логирование с возможностью добавления номера задачи и полной трассировки стека.
...
class AsyncApiClient(ApiClientAbstract):
    ...
    def __init__(self, limiter: Optional[RateLimiter] = None) -> None:
        ...
        Args:
            limiter: Лимитер запросов. Если не задан, то создается внутренний лимитер.
        """
    ...
    async def get_page(
        ...
        """Выполняет асинхронный HTTP-запрос с обработкой ошибок и ограничением числа запросов.

        Аргументы:
            url: URL для выполнения запроса.
            params: Параметры запроса.
            headers: Заголовки запроса.
            method: Метод HTTP-запроса (get, post и т.д.).
            limit_request: Ограничивать ли число запросов в секунду.
            timeout: Таймаут для запроса.
            json_format: Преобразовывать ли ответ в формат JSON.
            if_error_return: Возвращать ли ответ в случае ошибки.
            json_data: Данные для JSON-запроса.
            log_fails: Логировать ли текст ответа при ошибках.

        Returns: Ответ сервера или None в случае неудачи.
        """
